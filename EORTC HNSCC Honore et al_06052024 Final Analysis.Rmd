---
title: "EORTC HNSCC Honore et al_06052024 Final Analysis"
...
total_base <- sum(!is.na(circ_data$ctDNA.Base))
print(total_base)
circ_data$ctDNA.Base <- as.factor(circ_data$ctDNA.Base)
cont_table_base <- table(circ_data$Stage, circ_data$ctDNA.Base)
print(cont_table_base)

#Number of Pts at post-treatment time point - percentage positivity by stage
rm(list=ls())
setwd("~/Downloads") 
circ_data <- read.csv("OP-16933_HNSCC Clinical data.csv")
circ_data <- circ_data[circ_data$ctDNA.available=="TRUE",]
...
total_mrd <- sum(!is.na(circ_data$ctDNA.postTx))
print(total_mrd)
circ_data$ctDNA.postTx <- as.factor(circ_data$ctDNA.postTx)
cont_table_mrd <- table(circ_data$Stage, circ_data$ctDNA.postTx)
...
m1<-wilcox.test(ctDNA.Base.MTM ~ cStage, data=circ_data, na.rm=TRUE, paired=FALSE, exact=FALSE, conf.int=TRUE)
print(m1)
...
m2<-wilcox.test(ctDNA.Base.MTM ~ cT.Status, data=circ_data, na.rm=TRUE, paired=FALSE, exact=FALSE, conf.int=TRUE)
print(m2)
...
m3<-wilcox.test(ctDNA.Base.MTM ~ cN.Status, data=circ_data, na.rm=TRUE, paired=FALSE, exact=FALSE, conf.int=TRUE)
print(m3)
...
m4<-wilcox.test(ctDNA.Base.MTM ~ p16, data=circ_data, na.rm=TRUE, paired=FALSE, exact=FALSE, conf.int=TRUE)
print(m4)
```
